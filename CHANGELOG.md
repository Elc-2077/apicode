...
### Changed
- ⬆️ **`max_tokens` 10000 → 32000**（OpenAI 与 Anthropic 两分支）: 给推理/思考模型更充足的输出空间，进一步降低被截断的概率。
- 💬 **截断提示改为引导手动续写**: 会话历史本就跨轮保留（且已不会被空消息污染），所以任一轮若被截断或提前结束，**直接输入「继续」即可接着往下做**。截断提示语相应改为提示用户输入「继续」。（未采用「模型自动续写」方案，保持由用户掌控节奏。）
