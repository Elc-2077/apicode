...
function drawUI() {
  ...
  term.moveTo(1, 1, logo);

  if (currentApi) {
    term.moveTo(1, 8, `           当前站点: ${currentApi.name} | ${currentApi.baseUrl}`);
  }
...
function bindKeyEvents() {
  ...
  term.on('key', (name) => {
    ...
    if (name && name.startsWith('CTRL_')) return;
    if (name === 'LEFT') {
    ...
    if (name === 'a' && tabs[currentTab].key === 'switch') login();
    if (name === 'd' && tabs[currentTab].key === 'switch') deleteCurrentSite();
  });
}
...
async function start() {
  ...
  console.log('实时监控已启动，每5分钟自动更新数据源');
}
